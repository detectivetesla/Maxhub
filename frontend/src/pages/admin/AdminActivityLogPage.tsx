    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">Activity Log</h1>
            ...
            <div className="bg-white dark:bg-white/5 border border-slate-100 dark:border-white/5 rounded-[3rem] overflow-hidden shadow-sm">
                <div className="p-8 border-b border-slate-100 dark:border-white/5 flex flex-col md:flex-row gap-6 items-center">
                    <div className="relative flex-1 w-full group">
                        <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-[#2ECC71] transition-colors" />
                        <input
                            type="text"
                            placeholder="Search by event, message, or user..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full bg-slate-50 dark:bg-black/20 border border-transparent focus:border-[#2ECC71]/50 rounded-2xl py-4 pl-14 pr-6 outline-none transition-all font-bold text-sm"
                        />
                    </div>
                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                            {loading ? (
                                ...
                                <tr><td colSpan={5} className="px-10 py-12 text-center text-slate-400 font-bold">No activity matched your criteria.</td></tr>
                                ...
                                <tr key={log.id} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.01] transition-colors group">
                                            ...
                                            <div className={cn(
                                                ...
                                                log.type === 'auth' ? "bg-purple-500/10 text-purple-600 border-purple-500/20" :
                                                    log.type === 'system' ? "bg-amber-500/10 text-amber-600 border-amber-500/20" :
                                                        log.type === 'bundle' ? "bg-[#2ECC71]/10 text-[#2ECC71] border-[#2ECC71]/20" :
                                    ...
                                    <td className="px-8 py-6">
                                        <div className="flex flex-col">
                                            ...
                                            {log.message && <p className="text-[10px] text-slate-400 font-medium truncate max-w-xs">{log.message}</p>}
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                ...
                <div className="p-8 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02] flex items-center justify-between">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Showing latest activity sessions</p>
                    <button className="flex items-center gap-2 font-black text-[10px] text-[#2ECC71] uppercase tracking-widest hover:translate-x-1 transition-transform">
